...
import { updateTodo, deleteTodos } from "@/services/todos";
...
export function ActivityDisplay({ data, isLoading, refetch }: { data: Todo[]; isLoading: boolean; refetch: () => void }) {
	...
	const handleUpdate = useCallback(
		async (id: number, is_completed: boolean) => {
						...
						<Card key={todo.id}>
							...
							{todo.is_completed ? null : (
								<CardAction className="flex flex-row gap-2 w-full px-6">
									<Button onClick={() => handleUpdate(todo.id, todo.is_completed)} className="flex-1 bg-green-600 hover:!bg-green-700 duration-800 ease-in-out cursor-pointer">
										Selesai
									</Button>
